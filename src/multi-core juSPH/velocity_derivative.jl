...
function velocity_derivative(particles, int_fluid, h, d, r_c,alpha, c_0, g, rho_0, p_0, Xi,gamma,xMin,yMin,Nx,Ny,dx_x,dy_y)
    ...
    # initialisation
    dvelx_by_dt = SharedArray{Float64}(length(int_fluid),1)
    dvely_by_dt = SharedArray{Float64}(length(int_fluid),1)
    ...
    @inbounds @sync @distributed for a = 1 : length(int_fluid)   # loop over all fluid particles
        ...
        @inbounds for m = 1:length(fluidParticles_Neighors) 
