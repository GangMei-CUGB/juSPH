...
function boundary_update(particles, int_fluid, int_boundary, d,r_c, h, g,xMin,yMin,Nx,Ny,dx_x,dy_y)
    ...
    pressure = zeros(Float64,length(int_boundary),1)
    ...
    # Search for particles adjacent to the boundary particle in the fluid particle range 
        ...
        boundaryParticles_neighbors = zeros(Int,Max_number,length(int_boundary))
        # link list search
    ...
    for n = 1:length(int_boundary) # over all boundary particles
        ...
        for m = 1:length(boundaryParticles_Neighors) # interaction with all fluid particles in viscinity
